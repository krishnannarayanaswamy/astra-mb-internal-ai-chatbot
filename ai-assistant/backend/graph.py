...
from backend.constants import ASTRA_COLLECTION_NAME
...
about finanical documents. Remember the documents are in vietnamese language. Respond to user in the same language as he asked the question.

Generate a comprehensive and informative answer of 600 words or less for the \
given question based solely on the provided search results (extracted text from PDF). You must \
...
OPENAI_MODEL4_KEY = "openai_gpt_4_0"
ANTHROPIC_MODEL_KEY = "anthropic_claude_3_opus"
...
gpt_4_0 = ChatOpenAI(model="gpt-4o", temperature=0, streaming=True)
...
llm = gpt_3_5.configurable_alternatives(
    ...
    **{
        OPENAI_MODEL4_KEY: gpt_4_0,
        ANTHROPIC_MODEL_KEY: claude_3_opus,
        FIREWORKS_MIXTRAL_MODEL_KEY: fireworks_mixtral,
        GOOGLE_MODEL_KEY: gemini_pro,
        COHERE_MODEL_KEY: cohere_command,
    },
).with_fallbacks(
    [gpt_3_5, gpt_4_0, claude_3_opus, fireworks_mixtral, gemini_pro, cohere_command]
)
...
def get_retriever() -> BaseRetriever:
    ...
    return vstore.as_retriever(search_kwargs=dict(k=5))
